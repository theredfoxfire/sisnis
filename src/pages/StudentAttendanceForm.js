...
class StudentAttendanceForm extends Component {
          ...
          <Form size="large">
            <Segment stacked>
              ...
              <Form.Input
                ...
                defaultValue={studentAttendance.schedule.subjectName}
                disabled="disabled"
              />
              <h4>Murid :</h4>
              <Form.Input
                fluid
                placeholder="Murid "
                defaultValue={studentAttendance.student.studentName}
                disabled="disabled"
              />
              <h4>Catatan :</h4>
              <Form.Input
                fluid
                placeholder="Catatan "
                defaultValue={studentAttendance.notes}
                onChange={(e) =>
                  this.setState({
                    studentAttendance: {
                      ...studentAttendance,
              ...
              <h4>Hari :</h4>
              <DropdownSelect
                value={studentAttendance.prsenceStatus}
                placeholder="Pilih Status Kehadiran"
                onChange={(e, { value }) =>
                  this.setState({
                    studentAttendance: {
                      ...studentAttendance,
                      presenceOptions: value,
                    },
                  })
                }
                multiple={false}
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      !isEqual(prevProps.selectedStudentAttendance, selectedStudentAttendance)
    ) {
  ...
  _handleSubmit = () => {
    ...
    studentAttendanceID > 0
      ? putStudentAttendance({ ...studentAttendance }, studentAttendanceID)
      : postStudentAttendance({ ...studentAttendance });
  };

  _validate = () => {
    let { student, schedule, presenceStatus } = this.state.studentAttendance;
