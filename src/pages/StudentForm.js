...
class StudentForm extends Component {
            ...
            <Segment stacked>
              ...
              <Form.Input
                ...
                onChange={(e) =>
                  this.setState({
                    name: e.target.value,
                    serial: serial || selectedStudent.serial,
                  })
                }
              />
              <h4>Jenis Kelamin:</h4>
              ...
              * Tahun-bulan-tanggal
              ...
              <Link
                to={name !== "" ? "/student" : `/student-form/${studentID}`}
              >
                <Button
                  color="teal"
                  size="small"
  ...
  _handleSubmit = () => {
    let studentID = this.props.match.params.id;
    studentID > 0
      ? putStudent({ ...this.state }, studentID)
      : postStudent({ ...this.state });
  };

  _validate = () => {
    let { name, serial, gender, parentName, parentAddress, city, religion } =
      this.state;
    return (
      serial === "" ||
      name === "" ||
