...
export default class ExamForm extends Component {
        ...
        <Grid.Column stretched width={12}>
          ...
          <Form size="large">
            <Segment stacked>
              ...
              <Form.Input
                fluid
                placeholder="Nama Ujian/Ulangan/Tugas"
              ...
              <h4>Type Exam</h4>
              <DropdownSelect
                placeholder="Pilih type exam"
                onChange={(e, { value }) => this.setState({ examType: value })}
                multiple={false}
                options={examOptions}
              />
              <br />
              ...
              <Link
                to={
                  !this._validate()
                    ? `/teacher-subject-detail/${teacherSubject}`
                    : `/teacher-subject-exam-detail/${examID}/${teacherSubject}`
                }
              >
                <Button
                  color="teal"
  ...
  _handleSubmit = () => {
    ...
    let teacherSubject = this.props.match.params.teacherSubject;
    examID > 0
      ? putExam({ name: name, teacherSubject, examType, examDate }, examID)
      : postExam({ name: name, teacherSubject, examType, examDate });
  };
