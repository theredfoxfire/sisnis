...
class ExamTypeForm extends Component {
            ...
            <Segment stacked>
              ...
              <Form.Input
                ...
                onChange={(e) =>
                  this.setState({
                    name: e.target.value,
                    scale: scale || selectedExamType.scale,
                  })
                }
              />
              <h4>Skala Nilai :</h4>
              <Form.Input
                fluid
                placeholder="Skala Nilai"
                defaultValue={scaleValue}
                onChange={(e) =>
                  this.setState({
                    scale: e.target.value,
                    name: name || selectedExamType.name,
                  })
                }
              />
              <Link to={"/exam-type"}>
              ...
              <Link
                to={
                  name !== "" ? "/exam-type" : `/exam-type-form/${examTypeID}`
                }
              >
                <Button
                  color="teal"
  ...
  _handleSubmit = () => {
    ...
    let examTypeID = this.props.match.params.id;
    examTypeID > 0
      ? putExamType({ name: name, scale: scale }, examTypeID)
      : postExamType({ name: name, scale: scale });
  };
